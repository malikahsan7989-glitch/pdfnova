...
export default function DeletePdfPages() {
  ...
  const handleFile = async (files: File[]) => {
    ...
    try {
      ...
      setFile(null);
      setError(toFriendlyMessage(err, "delete-pages-validate"));
    } finally {
  ...
  const handleDelete = async () => {
    ...
    if (selected.size >= thumbnails.length) {
      setError("You can't delete every page — at least one page must remain in the PDF.");
      return;
    }
    ...
    try {
      ...
      const bytes = await deletePages(file, Array.from(selected));
      const blob = new Blob([bytes as unknown as BlobPart], { type: "application/pdf" });
      setResult({ blob, filename: safeFilename(`${file.name}-edited`, "pdf") });
    } catch (err) {
      setError(toFriendlyMessage(err, "delete-pages"));
    } finally {
      setProcessing(false);
    }
  };
  ...
  return (
    <div className="w-full max-w-3xl rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      <h2 className="text-2xl font-bold text-slate-900">Delete PDF Pages</h2>
      <p className="mt-2 text-sm text-slate-600">
        Select the pages you want to remove, then delete them from your document.
      </p>
      ...
      {processing && <ProgressBar label="Removing selected pages…" />}

      {result && !processing && (
        <ResultDownload
          message="Your edited PDF is ready."
        ...
        <div className="mt-6 flex flex-col gap-3 sm:flex-row">
          {!result && (
            <button
              ...
              className="flex-1 rounded-xl bg-red-600 px-5 py-3 font-semibold text-white transition hover:bg-red-700 disabled:cursor-not-allowed disabled:opacity-50"
            >
